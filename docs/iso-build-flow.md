...
- On Ubuntu 24.04, Colin OS also scans the installed `live-build` syslinux helpers after dependency installation and again immediately before `lb build`, patching any helper that still hardcodes `/root/isolinux` instead of the resolved bootloader source directory.
