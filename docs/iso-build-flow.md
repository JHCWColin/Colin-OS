...
- On Ubuntu 24.04, the packaged `live-build` syslinux stage can still expect legacy `/root/isolinux/*` paths. Colin OS prepares compatibility files before starting `lb build`, copying them into both the host `/root/isolinux` path and `config/includes.chroot/root/isolinux`, using `live-build` bootloader assets first and `isolinux` or `syslinux` package files as fallbacks.
- On Ubuntu 24.04, Colin OS first stages `/root/isolinux` from the installed `isolinux` and `syslinux` packages, then scans the installed `live-build` syslinux helpers after dependency installation and again immediately before `lb build`, patching any helper that still hardcodes `/root/isolinux` instead of the resolved bootloader source directory.
