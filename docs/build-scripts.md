...
- On Ubuntu 24.04 runners, `live-build` can still expect `/root/isolinux/isolinux.bin` and `/root/isolinux/vesamenu.c32` even though the `syslinux` packages install those files under `/usr/lib`. The wrapper now creates compatibility symlinks before `lb build`.
